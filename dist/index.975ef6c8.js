...
const showError = (message)=>{
    (0, _notiflixNotifyAio.Notify).failure(message);
};
...
const displayResults = (data)=>{
    if (data.length === 0) (0, _notiflixNotifyAio.Notify).failure("Oops there is no country with that name");
    else if (data.length === 1) showCountry(data[0]);
    else if (data.length > 10) (0, _notiflixNotifyAio.Notify).failure("Too many matches found. Please enter a more specific name");
    else if (data.length < 10 && data.length >= 2) showCountriesList(data);
};
...
const debounceSearchCountry = _.debounce(()=>{
    ...
    (0, _fetchCountriesJs.fetchCountries)(finalURL).then((data)=>{
        displayResults(data);
    });
}, DEBOUNCE_DELAY);
searchBtn.addEventListener("click", debounceSearchCountry);

},{"./fetchCountries.js":"fTQqv","notiflix/build/notiflix-notify-aio":"eXQLZ"}],"fTQqv":[function(require,module,exports) {
